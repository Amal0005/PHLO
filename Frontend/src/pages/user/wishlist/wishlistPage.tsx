...
import { Heart, Image as ImageIcon, Package as PackageIcon, Download, MapPin } from "lucide-react";
...
const WishlistPage: React.FC = () => {
    ...
    const fetchWishlistItems = useCallback(async () => {
        try {
            ...
            const itemIds = wishlistRes.data.map((item: { itemId: string }) => item.itemId);

            if (activeTab === "wallpaper") {
                const wpRes = await UserWallpaperService.getApprovedWallpapers(1, itemIds.length, {
                    ids: itemIds.join(","),
                });
    ...
    const handleRemoveFromWishlist = async (itemId: string) => {
        try {
            await WishlistService.toggle(itemId, activeTab);
            if (activeTab === "wallpaper") {
                setWallpapers((prev) => prev.filter((wp) => wp._id !== itemId));
            } else {
                setPackages((prev) => prev.filter((pkg) => pkg._id !== itemId));
            }
        } catch {
                        ...
                        <>
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                {wallpapers.map((wp) => (
                                    <div
                                        key={wp._id}
                                        className="bg-zinc-900 border border-white/10 rounded-2xl overflow-hidden hover:border-white/20 transition-all group aspect-[3/4] relative"
                                    >
                                        <div className="relative w-full h-full">
                                            ...
                                            <div className="absolute top-3 left-3 z-10 flex gap-1.5">
                                                <span
                                                    className={`px-2.5 py-1 rounded-full text-xs font-bold backdrop-blur-md border ${wp.price > 0
                                            ...
                                            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <button
                                                    onClick={() => handleRemoveFromWishlist(wp._id)}
                                                ...
                                                <div className="absolute bottom-3 left-3 right-3">
                                                    ...
                                                    <p className="text-gray-300 text-xs">
                                                        {typeof wp.creatorId === "object"
                                                            ? wp.creatorId.fullName
                                                            : ""}
                                                    </p>
                                                </div>
                                            </div>
                                    ...
                                    <div className="h-56 bg-zinc-800 relative overflow-hidden">
                                        ...
                                        <div className="absolute top-4 left-4">
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    handleRemoveFromWishlist(pkg._id);
                                                }}
