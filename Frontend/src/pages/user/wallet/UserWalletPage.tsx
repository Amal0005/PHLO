            ...
            <main className="max-w-7xl mx-auto px-6 pt-32 pb-20 space-y-12 animate-in fade-in duration-700">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
                    <div className="space-y-4">
                        ...
                        <p className="text-zinc-500 text-sm font-medium tracking-wide uppercase">Manage your funds and track your transactions</p>
                    </div>
                ...
                <div className="max-w-7xl mx-auto space-y-8">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-zinc-900/30 p-4 rounded-3xl border border-white/5 backdrop-blur-xl">
                        <div className="flex items-center gap-4 w-full md:w-auto px-4">
                            <h3 className="text-sm font-black uppercase tracking-[0.2em] whitespace-nowrap">Transaction History</h3>
                            <div className="h-4 w-px bg-white/10" />
                            <span className="text-xs font-bold text-zinc-500">{data?.totalTransactions || 0} Records Found</span>
                        </div>
                        ...
                        {loading ? (
                            ...
                            <div className="text-center py-24 bg-zinc-900/20 rounded-[3rem] border border-dashed border-white/10 space-y-6">
                                ...
                                <div className="space-y-2">
                                    <p className="text-lg font-black uppercase tracking-tight">No Transactions Yet</p>
                                    <p className="text-zinc-500 text-sm">Your financial activities will appear here</p>
                                </div>
                            </div>
                        ) : (
    ...
    return (
        <div className="group bg-zinc-900/40 hover:bg-zinc-900/60 p-6 rounded-[2rem] border border-white/5 hover:border-white/10 transition-all duration-300 flex items-center justify-between gap-6">
            <div className="flex items-center gap-6 flex-1">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-transform group-hover:scale-110 ${isCredit
                    ...
                    <div className="flex items-center gap-3 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                        ...
                        {tx.relatedName && (
                            <>
                                ...
                                <span className="text-zinc-600">From: {tx.relatedName}</span>
                            </>
                        )}
                    </div>
                </div>
            ...
            <div className="text-right space-y-1">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Reference: {tx.sourceId.slice(-8).toUpperCase()}</p>
            </div>
        </div>
    );
};
